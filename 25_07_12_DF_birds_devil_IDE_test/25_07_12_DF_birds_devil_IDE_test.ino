...
//RX extension (TX is 4.5 sec) - commented it out, seems to work
//next, comment out on other 2 heads, test
//doesn't work reliably - go to 1 second

// 7/11/notes
//next, shorten xmit time for "111' by making function "xmit3"
//test with all three heads -
//next, delete old notes, unused code 
//next, use to analyze ultrasonic/RF times
...
}    //________________________end of transmitter control thr digits
...
//added 7/12/25
myDFPlayer.playMp3Folder(27);  //_____________ I'm only a talking head, but
dur=40;
servo(val0, dur);
myDFPlayer.pause();
...
/*
Serial.println();
Serial.print(" start Giant Heads");
Serial.println();
delay(100); 
*/
...
if (u >4) {u=1;}  //__________________________________segment "u" player 
if (distance < dist) {
myDFPlayer.volume(30);
myDFPlayer.playMp3Folder(u);   //first set of birds
dur=50;
servo(val0, dur);

myDFPlayer.playMp3Folder(u+14);  //second set of birds
dur=50;   //from 40 4/3
servo(val0, dur);
myDFPlayer.pause();


if (u==1) {  /////_________________________________ segment "u" transmit
...
for (i=0; i<300; i++) {     //from 500 on 4/22/23
vx= digitalRead(D0);
vy=digitalRead(D1);
vz= digitalRead(D2);
...
//from 2 sec to 1 sec 7/15/25
//____________________________________________________extend detect time if any signal is detected
...
if(b==2){
myDFPlayer.playMp3Folder(22);  //_____________ did somebody let the chordettes out?
dur=30;   
servo(val0, dur);
myDFPlayer.pause();
}
...
if(s==1) {
myDFPlayer.playMp3Folder(9); //__________________ me neither, but here's a falcon
dur=100;
servo(val0, dur);
myDFPlayer.pause();
}
...
if(s==2) {
myDFPlayer.playMp3Folder(10); //_______________me neither,but here's a woodpecker
dur=100;
servo(val0, dur);
myDFPlayer.pause();
}
...
dur=95;  
//  myDFPlayer.volume(30);  //error fixed 7/3/25
servo(val0, dur);
myDFPlayer.pause();
...
if(VZ==HIGH&&VY==HIGH&&VX==HIGH) {   //____________________________ 1 1 1 code detect "c" loop
