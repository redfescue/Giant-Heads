// 7/15/notes
//shorten RX extend time from 2 sec to 1 sec
...
int u = 41; // for counting the ultrasonic sensor loop
...
Serial.println();
Serial.println(F("Giant Heads Dogs"));
...
//added 7/11/25
myDFPlayer.playMp3Folder(28);  //_____________ I'm only a talking head, but
dur=70;
servo(val0, dur);
myDFPlayer.pause();
...
if (u >44) {u=41;}  //__________________________________segment "u" player 
if (distance < dist) {
myDFPlayer.volume(30);
myDFPlayer.playMp3Folder(u);   //dogs
if (u==41) {dur=52;}   
if (u==42) {dur=62;}
if (u==43) {dur=60;}
if (u==44) {dur= 80;}
...
if (u==41) {  /////_________________________________ segment "u" transmit
...
if(b==2){
myDFPlayer.playMp3Folder(20);  //_____________ Is that those damn birds again?
dur=40;
servo(val0, dur);
myDFPlayer.pause();
}
...
if(s==1) {
myDFPlayer.playMp3Folder(23); //__________________ didn't hear anything, but here's a nice doggie
dur=50;
servo(val0, dur);
myDFPlayer.pause();
myDFPlayer.playMp3Folder(30); //__________________ barking dog
dur=30;
servo(val0, dur);
myDFPlayer.pause();
}
...
if(s==2) {
myDFPlayer.playMp3Folder(24); //_______________Not me, but here's a weird dog
dur=40;
servo(val0, dur);
myDFPlayer.pause();
myDFPlayer.playMp3Folder(29); //__________________ cat meow
dur=30;
servo(val0, dur);
myDFPlayer.pause();
